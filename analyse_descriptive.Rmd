...
ggplot(df_age, aes(x = factor(age), y = freq, fill = age)) +
  ...
  geom_text(aes(y = freq, label = paste(round(freq, 2), "%")), vjust = -0.25) +
...
ggplot(df_conta, aes(x = factor(conta), y = freq, fill = conta)) +
  geom_bar(stat="identity", width = 0.7) +
  geom_text(aes(y = freq, label = paste(round(freq, 2), "%")), hjust = 1.2) +
...
ggplot(df_motif, aes(x = factor(motif), y = freq, fill = motif)) +
  geom_bar(stat="identity", width = 0.7) +
  geom_text(aes(y = freq, label = paste(round(freq, 2), "%")), hjust = 1.25) +
...
Le profil type d'un patient est une femme ayant entre 30 et 40 ans, étant contaminé par voie sexuelle (hétérosexuelle) mais n'ayant pas atteint le stade du sida. Elle a reçu le traitement 2 IN+1IP mais ne le suit pas correctement. Son niveau de CD4 passe de moins de 200 à plus de 350 durant le traitement, sa charge virale diminue, passant de 4-5 à moins de 1,7. Elle a tendance à arrêter son traitement pour intolérance/toxicité.
...
library(reshape2)
dcast(df_motif_trt, typetrt~motif)
...
barplot(t(prop.table(tableau2[1:2,],1)*100),legend.text = TRUE, beside = TRUE, xlab="type de traitement", ylab = "Fréquence (en %)", main = "Fréquence du type d'arrêt en fonction du sexe du patient", ylim = c(0,40))
```
...
barplot(t(prop.table(tableau3[1:4,],1)*100),legend.text = TRUE, beside = TRUE, xlab="classe d'âge", ylab = "Fréquence (en %)", main = "Fréquence du type d'arrêt en fonction de l'âge du patient", ylim = c(0,50))
```
...
barplot(t(prop.table(tableau4[1:2,],1)*100),legend.text = TRUE, beside = TRUE, xlab="Observance du patient", ylab = "Fréquence (en %)", main = "Fréquence du type d'arrêt en fonction de l'âge l'observance du patient", ylim = c(0,50))
```
...
barplot(t(prop.table(tableau5[1:2,],1)*100),legend.text = TRUE, beside = TRUE, xlab="stade du patient", ylab = "Fréquence (en %)", main = "Fréquence du type d'arrêt en fonction du stade de la maladie du patient", ylim = c(0,40))
```
...
tableau7
barplot(t(prop.table(tableau7[1:3,],1)*100),legend.text = TRUE, beside = TRUE, xlab="niveau de CD4", ylab = "Fréquence (en %)", main = "Fréquence du type d'arrêt en fonction du niveau de CD4, au début, du patient", ylim = c(0,40))
```
...
tableau8
barplot(t(prop.table(tableau8[1:3,],1)*100),legend.text = TRUE, beside = TRUE, xlab="niveau de CD4", ylab = "Fréquence (en %)", main = "Fréquence du type d'arrêt en fonction du niveau de CD4, à la fin, du patient", ylim = c(0,40))
```
...
tableau9
barplot(t(prop.table(tableau9[1:3,],1)*100),legend.text = TRUE, beside = TRUE, xlab="charge de CV", ylab = "Fréquence (en %)", main = "Fréquence du type d'arrêt en fonction de la charge virale, au début, du patient", ylim = c(0,40))
```

Les patients ayant eux une charge virale "faible" au début du traitement ont été plus tolérent au traitement et ont eux leur traitement simplifier moins souvent que la moyenne, respectivement -11% et -5%. Par contre il y a eu plus d'échec thérapeutique, +4%. Les patientsavec une charge virale très élevés ont eux plus souvent une simplification de leur traitement, +7 points, et moins d'échec thérapeutique, -4 points.
...
Les patients qui ont une charge virale inférieur à 1,7 ont arrêté plus souvent que la moyenne dû à une simplification du traitement, 15 points de plus que la moyenne. A l'inverse, ils ont rarement arrêté à cause d'un échec thérapeutique et pour un problème d'observance,  10 et 4 points demoins que la moyenne. On retrouve le cas inverse pour les patients ayant une charge virale supérieur à 4. Ils ont un grand nombre d'échec thérapeutique et de problème d'observance, +18 et +10 points par rapport à la moyenne, et moins de simplification, -19 points (il n'y en a pas).

...
```{r}
tablefreq_motif_trt <- addmargins(table(base_vih$typetrt, base_vih$motif), margin = 1)
tablefreq_motif_trt
...
```{r}
tablefreq_motif_sexe <- addmargins(table(base_vih$sexe, base_vih$motif), margin = 1)
tablefreq_motif_sexe
prop.table(tablefreq_motif_sexe, 1) * 100

df_motif_sexe <- base_vih %>%
  filter(!is.na(motif)) %>%
...
```{r}
tablefreq_motif_age <- addmargins(table(base_vih$age, base_vih$motif), margin=1)
tablefreq_motif_age
prop.table(tablefreq_motif_age, 1) * 100

df_motif_age <- base_vih %>%
  filter(!is.na(motif)) %>%
  mutate(age = factor(age, labels = c("[18;30[", "[30;40[", "[40;50[", "[50;60["))) %>%
  group_by(age, motif) %>%
  summarise(count=n()) %>% 
...
```{r}
tablefreq_motif_observ <- addmargins(table(base_vih$observ, base_vih$motif), margin=1)
tablefreq_motif_observ
prop.table(tablefreq_motif_observ, 1) * 100

df_motif_observ <- base_vih %>%
  filter(!is.na(motif)) %>%
  mutate(observ = factor(observ, labels = c("non", "oui"))) %>%
...
```{r}
tablefreq_motif_sida <- addmargins(table(base_vih$sida, base_vih$motif), margin=1)
tablefreq_motif_sida
prop.table(tablefreq_motif_sida, 1) * 100

df_motif_sida <- base_vih %>%
  filter(!is.na(motif)) %>%
  filter(!is.na(sida)) %>%
...
Les patients ayant atteint le stade du sida ont arrêtaient plus souvent dû a une intolérence ou un échec thérapeutique, respectivement 4.3 et 2.3 point au dessus de la moyenne.

...
Les femmes arrêtent leur traitement plus souvent pour d'autres raison que les hommes, $34\%$ contre $25\%$. Sinon que le patients soit une femme ou un homme les proportions d'arrêts sont presque identique quelque soit le motif. 
...
```{r}
tablefreq_motif_conta <- addmargins(table(base_vih$conta, base_vih$motif), margin = 1)
prop.table(tablefreq_motif_conta, 1) * 100
tablefreq_motif_conta

df_motif_conta <- base_vih %>%
  filter(!is.na(motif)) %>%
  filter(!is.na(conta)) %>%
...
```{r}
chisq.test(tablefreq_motif_conta)
```
...
```{r}
tablefreq_motif_cd4b1 <- addmargins(table(base_vih$cd4b1, base_vih$motif),  margin = 1)
prop.table(tablefreq_motif_cd4b1,1) * 100
tablefreq_motif_cd4b1

df_motif_cd4b1 <- base_vih %>%
  filter(!is.na(motif)) %>%
  filter(!is.na(cd4b1)) %>%
  mutate(cd4b1 = factor(cd4b1, labels = c("<200", "200:350",">350"))) %>%
...
```{r}
tablefreq_motif_cd4b2 <- addmargins(table(base_vih$cd4b2, base_vih$motif), margin = 1)
prop.table(tablefreq_motif_cd4b2, 1) * 100
tablefreq_motif_cd4b2

df_motif_cd4b2 <- base_vih %>%
  filter(!is.na(motif)) %>%
  filter(!is.na(cd4b2)) %>%
  mutate(cd4b2 = factor(cd4b2, labels = c("<200", "200:350",">350"))) %>%
...
```{r}
tablefreq_motif_cvb1 <- addmargins(table(base_vih$cvb1, base_vih$motif), margin = 1)
prop.table(tablefreq_motif_cvb1, 1) * 100
tablefreq_motif_cvb1

df_motif_cvb1 <- base_vih %>%
  filter(!is.na(motif)) %>%
  filter(!is.na(cvb1)) %>%
...
```{r}
tablefreq_motif_cvb2 <- addmargins(table(base_vih$cvb2, base_vih$motif), margin = 1)
prop.table(tablefreq_motif_cvb2, 1) * 100
tablefreq_motif_cvb2

df_motif_cvb2 <- base_vih %>%
  filter(!is.na(motif)) %>%
  filter(!is.na(cvb2)) %>%

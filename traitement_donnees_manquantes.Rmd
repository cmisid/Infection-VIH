...
On décide de supprimer les lignes ou il y a des valeurs manquantes pour les facteurs :  
- `sida`
- `cd4b1`
- `cd4b2`
- `cvb1`
...
```{r}
base_vih_df <- base_vih_df[-find_index_for_NaN_combinations(4),]
base_vih_df <- base_vih_df[-find_index_for_NaN_combinations(3),]
...
train_rpart <- train(sida ~ age + sexe + observ + conta + typetrt + arret_trt + debtrtc, data=train_df,
method="rpart", trControl=fitControl, tuneGrid=rpart_grid)
train_rpart
```
...
## Stratégie d'interpolation pour les facteurs liés au taux de CD4 et de charge virale

### Remplacement des valeurs manquantes pour CD4 à l'initiation du traitement

Nous avons ici 210 données manquantes pour le facteur `cd4b1`. Nous décidons de mettre en oeuvre une méthode d'apprentissage pour remplacer les valeurs manquantes.
```{r}
...
# Let's train the Decision Tree classifier
train_rpart <- train(cd4b1 ~ sida + delta_dtrt_dsero + debtrtc + age + sexe + observ + conta, 
...
```{r}
# List of indexes where the modality for `cd4b1` is NaN
na_index <- which(is.na(base_vih_df$cd4b1))
# For each index, find the value in predictions array and assign it to our previous NaN value
for (i in 1:length(na_index)) {
 base_vih_df$cd4b1[na_index[i]] <- preds[i] 
}
```
...
# Let's train the Decision Tree classifier
train_rpart <- train(cvb1 ~ sida + delta_dtrt_dsero + debtrtc + age + sexe + observ + conta, 
   data=train_df, method="rpart", 
   trControl=fitControl, 
   tuneGrid=rpart_grid)
...
# Let's train the Decision Tree classifier
train_rpart <- train(cvb2 ~ sida + delta_dtrt_dsero + debtrtc + age + sexe + observ + conta, 
   data=train_df, method="rpart", 
   trControl=fitControl, 
   tuneGrid=rpart_grid)
